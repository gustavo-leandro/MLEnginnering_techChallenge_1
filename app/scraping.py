		...
		for link in ls_links:
			...
			for row in info_table.find_all("tr"):
				if row.th.get_text(strip=True) == "UPC":
					...
					b_price_excl_tax = float(re.sub(r"[^0-9.]", "", row.td.get_text(strip=True)))
				elif row.th.get_text(strip=True) == "Price (incl. tax)":
					b_price_incl_tax = float(re.sub(r"[^0-9.]", "", row.td.get_text(strip=True)))
					...
					b_num_available = int(re.search(r'\d+', availability_text).group()) if re.search(r'\d+', availability_text) else 0
